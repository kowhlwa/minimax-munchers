...
class PlayerBoard:
    ...
    def get_copy(self, reverse_perspective = False):
        return PlayerBoard(self.board.get_copy(), self.opponent_parity)

    def can_move(self, move: Action, moves_this_turn: int = 0, opponent=False):
        player = self.get_player(self.player_parity, opponent)
        ...
        if target_loc == self.board.get_opponent(parity_query).loc:
            return target_cell.owner_parity != parity_query * -1
        
        if move.place_beacon:
            if not self.can_place_beacon():
                return False
    ...
    def can_place_beacon(self, origin, opponent = False):
        ...
        player = self.get_player(parity_query)
        cell = self.board.cells[origin.r][origin.c]
        if cell.owner != parity_query:
            return False
        if cell.beacon_owner is not None:
            return False
