...
DANGER_STRICT = 3
DANGER_MODERATE = 1
MIN_REGEN_BASE = 6  # same as v13 — prevents farm/rush oscillation
...
class PlayerController:
    ...
    Key changes from v13:
      - BEACON DEPLOYMENT (BEACON_COST=0, free permanent territory anchors).
        Beacons cannot be removed by opponent. Beacon on hill = locked hill cell.
        Deployed opportunistically on every qualifying move (no wasted turn).
      - BEACON TELEPORTATION: jump between own beacons for instant defense/offense.
        Used in emergency (contested hill) or rush (beacon near target hill).
      - TRIPLE MOVES: up to 3 moves/turn in rush/emergency/contest phases.
        Allows reaching hills ~33% faster.
      - BETTER BID: 5/15/25/35 based on hill distance (was 5/10/15).
        More aggressive initiative grab when hills are close.
      - ENDGAME TERRITORY phase: after round 750 with hill lead, flood-fill territory
        to maximize tiebreak score.
      - POWERUP grab range increased to 4 steps (was 2).
      - MIN_REGEN_BASE lowered to 5 (was 6).
      - Hill defense scoring raised in _choose_hill_target.
    """
        ...
        if phase in ("emergency", "contest", "rush"):
            ...
            if target:
                ...
                if direction:
                    ...
                    # Second move (no beacon — board state stale after move 1)
                    if td and td > 2 and stamina_after_m1 >= GameConstants.EXTRA_MOVE_COST + buf + 10:
                        ...
                        if d2 and not self._is_step_dangerous(board, np2 + d2, player_parity):
                            ...
                            # Third move (v14 new, no beacon)
                            if (td > 3
                                    and stamina_after_m1 >= GameConstants.EXTRA_MOVE_COST * 2 + buf + 10):
                                ...
                                if d3 and not self._is_step_dangerous(board, np3 + d3, player_parity):
                                    ...
                                    stamina_after_m1 -= GameConstants.EXTRA_MOVE_COST * 2
    ...
    def _choose_hill_target(
        ...
        # Defend our hills under attack (v14: raised urgency scoring)
        for hill_id in player.controlled_hills:
            hill = board.hills[hill_id]
            opp_cells = sum(
                1 for hloc in hill.cells
            ...
            if best_def_loc is not None:
                ...
                # v14: raised from 200+urgency*200 to 300+urgency*300
