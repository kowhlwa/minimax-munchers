...
class PlayerController:
    """
    actualai — aggressive hill-racing bot tuned to beat alphapaint.

    Key design principles:
    - Movement > painting: avg 1.6+ moves/turn, minimize trail paint when racing
    - Bid 16 to win first-move advantage over alphapaint (bids 15)
    - Lower stamina buffers to enable 2-3 move turns consistently
    - Only paint hill cells and territory when all hills are claimed
    - Beacon-aware pathfinding: walks through opponent beacons
    - Domination finisher: erase+paint to close out games
    - Fixed: collision safety re-checked after fallback moves
    """
    ...
    def bid(self, board: Board, player_parity: int, time_left) -> int:
        # Bid 16 to beat alphapaint's 15 and win first-move advantage.
        # Costs 16 stamina but going first is worth far more — lets us
        # reach hills before opponent and claim territory proactively.
        return 16
    ...
    def play(
        ...
        # === DOMINATION FINISHER: check if erase+paint can win the game ===
        dom_finish = self._try_domination_finish(board, pp, me, opp, stamina)
        if dom_finish is not None:
            return dom_finish
        ...
        early_game = board.current_round < 30

        # === ESCAPE: on enemy territory near opponent — but NOT if near a hill or early game ===
        # In early game, don't waste turns fleeing — focus on reaching first hill
        if not near_any_hill and not early_game and cell.owner_parity == -pp and mon['proximity'] <= 3:
            esc = self._escape_dir(board, pos, opp.loc, pp)
            if esc:
                return [Action.Move(esc)]

        # === LOW STAMINA RETREAT — but NOT if near a hill or early game ===
        if not near_any_hill and not early_game and stamina < 35 and cell.owner_parity != pp:
        ...
        # === STAMINA BUDGET ===
        # Aggressive buffers — prioritize movement over hoarding stamina.
        # Alphapaint-style: spend stamina to move, not to save.
        racing = (not on_target_hill and td > 2 and not contested
                  and neutral_hills_exist)

        if maze_map:
            buf = 20 if td > 3 else 10
        elif racing:
            # When racing to uncaptured hills, minimize buffer to allow more moves
            buf = 15
        elif td > 5:
            buf = 25
        elif td > 2:
        ...
        # Key insight: alphapaint paints 0.8-1.2/turn and wins. We paint 1.2-1.8 and lose.
        # Only paint trail when all hills claimed or in danger. When racing, skip trail.
        paint_territory_en_route = all_hills_claimed or (danger_zone and not racing)

        trail_layers = 2 if danger_zone else 1

        # === PRE-MOVE PAINTING ===
        # Always paint adjacent hill cells — this is how we capture hills
        # Only reinforce when ON the target hill; don't waste stamina
        # reinforcing a captured hill we're about to leave
        actions, stamina = self._paint_hill_cells(board, pp, pos, actions, stamina, painted, hill_buf, reinforce if on_target_hill else False, max_layers)

        # Trail painting: ONLY when all hills claimed, in a maze, or danger zone
        # When racing to hills, DO NOT waste stamina on trail paint
        ...
        if d1:
            ...
            if paint_territory_en_route and not racing:
                actions, stamina = self._paint_trail(board, pp, p1, actions, stamina, painted, buf, target, trail_layers)

            # 2nd move — lower threshold to encourage multi-move turns
            move2_threshold = GameConstants.EXTRA_MOVE_COST + (hill_buf if on_target_hill else buf)
            if on_target_hill:
                ...
                if p1_hill_id != 0:
                    ...
                    if hill_obj_now and hill_obj_now.controller_parity != pp:
                        ...
                        if next_cell and next_cell != p1:
                            target = next_cell
            if (td > 1 or on_target_hill) and stamina >= move2_threshold:
                d2 = _pick_step(p1)
                if d2 and (targeting_hill or not self._is_dangerous_monitor(board, p1 + d2, opp, pp)):
                    ...
                    # No trail paint after 2nd move when racing — save stamina for 3rd move
                    if paint_territory_en_route and not racing:
                        actions, stamina = self._paint_trail(board, pp, p2, actions, stamina, painted, max(buf - 10, 10), target, trail_layers)

                    # 3rd move — more aggressive threshold
                    if not maze_map and (td > 4 or racing) and stamina >= GameConstants.EXTRA_MOVE_COST * 2 + max((hill_buf if on_target_hill else buf) - 10, 5):
                        d3 = _pick_step(p2)
        ...
        # Post-move trail: only on hills or when all hills claimed
        if board.cells[final.r][final.c].hill_id != 0 and stamina > 25:
            actions, stamina = self._paint_trail(board, pp, final, actions, stamina, painted, 15, target, trail_layers)
        elif paint_territory_en_route and not racing:
            actions, stamina = self._paint_trail(board, pp, final, actions, stamina, painted, 15, target, trail_layers)
        ...
        if not any(isinstance(a, Action.Move) for a in actions):
            ...
            # RE-CHECK safety after fallback — this was the collision bug.
            # _ensure_safe_ending may strip moves, fallback adds an unchecked one.
            final2 = self._simpos(board, pos, actions)
            if not self._is_collision_safe(board, final2, pp, opp, painted):
                # Try to find a safe fallback direction
                safe_fb = self._safe_fallback_move(board, pp, me, opp, painted)
                if safe_fb:
                    # Replace the last move with the safe one
    ...
    def _is_collision_safe(self, board, loc, pp, opp, painted=None):
        """Check if ending a turn at loc is safe from collision.
        Uses opponent monitor for accurate threat assessment.
        `painted` dict tracks cells we've painted this turn (ownership changed)."""
        if board.oob(loc):
            return False
        c = board.cells[loc.r][loc.c]
        mon = self._opp_monitor

        # Check if we painted this cell during our turn — it's now ours
        cell_is_ours = (c.owner_parity == pp and c.beacon_parity != -pp)
        if not cell_is_ours and painted and loc in painted:
            # We painted it this turn: if it was neutral, it's now ours
            if c.owner_parity == 0 and c.beacon_parity != -pp:
                cell_is_ours = True
    ...
    def _should_erase_step(self, board, pp, loc, large_map, stamina, buf, opp, target_hill_obj):
        ...
        Returns True if the cell has thick opponent paint and erase is worthwhile.
        Works on hill cells AND non-hill cells on larger/spacious maps."""
        if board.oob(loc):
            return False
        c = board.cells[loc.r][loc.c]

        # Only erase opponent-owned cells (no beacons — erase doesn't clear beacons)
        if c.owner_parity != -pp or c.beacon_parity != 0:
            return False
        ...
        # Check proximity to domination — more aggressive erase when close to winning
        total_hills = len(board.hills)
        me = board.get_player(pp)
        dom_needed = math.ceil(total_hills * GameConstants.DOMINATION_WIN_THRESHOLD) if total_hills > 0 else 999
        close_to_dom = len(me.controlled_hills) >= dom_needed - 1 and total_hills > 1

        # On hills: erase thick layers to capture faster
        if is_hill:
            if close_to_dom and layers >= 2 and stamina >= erase_cost + buf + 10:
                return True  # Aggressive erase when close to domination
            if large_map and layers >= 2 and stamina >= erase_cost + buf + 20:
                return True
            if not large_map and layers >= 3 and stamina >= erase_cost + buf + 30:
                return True
            return False

        # Non-hill cells on large/spacious maps: erase thick opponent paint
        # when opponent is not in collision range (safe to spend stamina)
        if large_map and layers >= 3:
            opp_dist = self._md(loc, opp.loc)
            if opp_dist > self._opp_monitor['threat_range'] + 2:
                if stamina >= erase_cost + buf + 30:
                    return True
    ...
    def _pick_hill_cell_to_claim(self, board, pp, hill, my_dist, contested, large_map, opp, stamina):
        ...
        for hloc in hill.cells:
            ...
            # Can't paint cells with opponent beacons — skip them
            if hc.beacon_parity == -pp:
                continue
    ...
    def _edge_hill_step(self, board, start, target, pp, opp_loc, hill_obj):
        ...
        for d in Direction.cardinals():
            ...
            if nxt == opp_loc and c.owner_parity != pp:
                continue
            # Hostile cells (opponent paint or beacon) near opponent are risky
            cell_hostile = (c.owner_parity == -pp or c.beacon_parity == -pp)
            if cell_hostile and self._md(nxt, opp_loc) <= 1:
                continue
            ...
            if c.owner_parity == pp and c.beacon_parity != -pp:
                score += 30
            elif c.beacon_parity == -pp:
                score -= 20  # opponent beacon
        ...
        for hid, hill in board.hills.items():
            ...
            for hloc in hill.cells:
                ...
                if hc.owner_parity == pp:
                    ...
                    if hc.owner_parity == -pp:
                        opp_cells += 1
                    # Only target cells we can actually paint (no opponent beacons)
                    if hc.beacon_parity != -pp and d < nearest_dist:
                        nearest_dist = d
                        nearest_loc = hloc
            ...
            # Skip hills where opponent beacons make capture impossible
            capturable_cells = hs - opp_beacon_cells
            if our_cells + (capturable_cells - our_cells) < needed:
                continue  # can't ever reach >50% even if we paint all non-beacon cells
            ...
            if our_hills == 0:
                ...
                contested_bonus = 0
                if hill.controller_parity == 0 and opp_nearest < 999:
    ...
    def _try_domination_finish(self, board, pp, me, opp, stamina):
        """Check if we can win by domination this turn via erase+paint combos.
        If we control all but one hill and that hill is within reach, go all-in."""
        total_hills = len(board.hills)
        if total_hills < 2:
            return None
        dom_needed = math.ceil(total_hills * GameConstants.DOMINATION_WIN_THRESHOLD)
        ...
        # Only attempt when 1 hill away from domination
        if our_hills < dom_needed - 1:
            return None

        pos = me.loc
        ERASE_COST = GameConstants.ERASE_STEP_EXTRA_COST
        PAINT_COST = GameConstants.PAINT_STAMINA_COST
        MOVE_COST = GameConstants.EXTRA_MOVE_COST
        ...
        for hid, hill in board.hills.items():
            ...
            hs = len(hill.cells)
            needed = math.ceil(hs * GameConstants.HILL_CONTROL_THRESHOLD) + 1
            our_cells = sum(1 for h in hill.cells
                           if board.cells[h.r][h.c].owner_parity == pp)
            cells_needed = needed - our_cells
            if cells_needed <= 0:
                continue  # already have enough, just needs tick?

            # Find claimable cells adjacent to us (erase+paint or just paint)
            claimable_now = []
            for hloc in hill.cells:
                hc = board.cells[hloc.r][hloc.c]
                if hc.owner_parity == pp or hc.beacon_parity == -pp:
                    continue
                md = self._md(pos, hloc)
                if md > 3:
                    continue  # too far for 1-turn finish

                if hc.owner_parity == 0 and md == 1:
                    # Neutral adjacent: just paint it
                    claimable_now.append(('paint', hloc, PAINT_COST))
                elif hc.owner_parity == -pp and md == 0:
                    # Standing on opponent cell: it becomes neutral after we move off
                    # Can paint from adjacent after moving
                    pass
                elif hc.owner_parity == -pp:
                    # Opponent cell: need to step on it (erase) then paint from next
                    # Cost: move + erase + paint
                    claimable_now.append(('erase_step', hloc, ERASE_COST))

            if len(claimable_now) < cells_needed:
                continue

            # Try to build an action sequence that captures enough cells
            # Simple case: all needed cells are adjacent and neutral (just paint)
            paints_only = [c for c in claimable_now if c[0] == 'paint']
            if len(paints_only) >= cells_needed:
                total_cost = cells_needed * PAINT_COST
                if stamina >= total_cost + 10:
                    actions = []
                    for i in range(cells_needed):
                        actions.append(Action.Paint(paints_only[i][1]))
                    # Must include a move for valid turn
                    # Step onto a hill cell we own or neutral
                    for d in Direction.cardinals():
                        nxt = pos + d
                        if board.oob(nxt):
                            continue
                        ...
                        if c.is_wall:
                            continue
                        if c.owner_parity == pp or c.owner_parity == 0:
                            actions.append(Action.Move(d))
                            return actions
                    # If can't find safe move, just move anywhere
                    for d in Direction.cardinals():
                        nxt = pos + d
            ...
            # Case: need erase step + paint behind
            # Move onto opponent cell with erase (becomes neutral), then paint from new pos
            erase_targets = [c for c in claimable_now if c[0] == 'erase_step']
            if cells_needed == 1 and len(erase_targets) >= 1:
                for _, eloc, _ in erase_targets:
                    md = self._md(pos, eloc)
                    if md == 1:
                        # Step with erase (cell becomes neutral/0), then paint from new position
                        for d in Direction.cardinals():
                            if pos + d == eloc:
                                erase_cost = ERASE_COST + PAINT_COST
                                if stamina >= erase_cost + 10:
                                    acts = [Action.Move(d, move_type=MoveType.ERASE)]
                                    # After erase, cell is neutral. Paint it from adjacent
                                    # Actually after erase move, we're ON the cell.
                                    # We need to paint an adjacent hill cell instead, or
                                    # step further and paint back.
                                    # Actually erase sets the cell to 0. We then paint adjacent.
                                    # But we need to paint the cell we just erased — can't paint
                                    # at distance 0. Need to move off and paint back.
                                    # Move + erase, then move again, then paint back
                                    move2_cost = MOVE_COST
                                    if stamina >= ERASE_COST + move2_cost + PAINT_COST + 5:
                                        # Find adjacent cell to step to
                                        for d2 in Direction.cardinals():
                                            nxt2 = eloc + d2
                                            if board.oob(nxt2) or nxt2 == pos:
                                                continue
                                            c2 = board.cells[nxt2.r][nxt2.c]
                                            if c2.is_wall:
                                                continue
                                            if self._md(nxt2, eloc) == 1:
                                                acts.append(Action.Move(d2))
                                                acts.append(Action.Paint(eloc))
                                                return acts
                                break
    ...
    def _ensure_safe_ending(self, board, pp, start_pos, opp, actions, stamina, painted):
        ...
        def _is_safe(loc):
            return self._is_collision_safe(board, loc, pp, opp, painted)
        ...
        # When opponent is >3 cells from our destination and we're not on their
        # territory, accept the position to avoid bouncing in place.
        # Pre-paint if possible for extra safety.
        opp_dist_to_final = self._md(final, opp.loc)
        if opp_dist_to_final > 3 and fc.owner_parity != -pp:
        ...
        for d in Direction.cardinals():
            ...
            if c.owner_parity == pp and c.beacon_parity != -pp:
                ...
                score = -200  # opponent beacon = their territory
            elif nd > opp_reach:
                score = 100
            elif c.owner_parity == 0:
                score = -50
    ...
    def _step_toward(self, board, start, target, pp, opp_loc, hill_target=False):
        ...
        Beacon cells are walkable (not walls) — treated like opponent paint for danger."""
        if start == target:
            return None
        ...
        for danger_r in danger_levels:
            ...
            for d in Direction.cardinals():
                ...
                if nxt == opp_loc and c.owner_parity != pp:
                    continue
                # Beacon cells count as opponent-owned for danger avoidance
                cell_hostile = (c.owner_parity == -pp or c.beacon_parity == -pp)
                if danger_r > 0 and cell_hostile and self._md(nxt, opp_loc) <= danger_r:
                    continue
                visited.add(nxt)
                if nxt == target:
                    return d
    ...
    def _collision_aware_step(self, board, start, target, pp, opp_loc):
        ...
        for d in Direction.cardinals():
            ...
            if c.owner_parity == pp and c.beacon_parity != -pp:
                ...
                safety = -60  # opponent beacon = opponent-owned territory
            elif c.owner_parity == -pp:
                safety = -60
    ...
    def _safe_hill_step(self, board, start, target, pp, opp_loc):
        ...
        for d in Direction.cardinals():
            ...
            if c.is_wall:
                continue
            # Beacon/opponent cells near opponent are risky but not impassable
            cell_hostile = (c.owner_parity == -pp or c.beacon_parity == -pp)
            if cell_hostile and self._md(nxt, opp_loc) <= 1:
                continue
            score = -self._md(nxt, target) * 10
            if c.owner_parity == pp and c.beacon_parity != -pp:
                ...
                score -= 15  # opponent beacon = opponent territory
            elif c.owner_parity == 0:
                score += 5
            elif c.owner_parity == -pp:
                score -= 10
    ...
    def _fallback_move(self, board, pp, me, opp):
        ...
        for d in Direction.cardinals():
            ...
            if c.owner_parity == pp and c.beacon_parity != -pp:
                ...
                p = -25  # opponent beacon = opponent territory, walkable but risky
            elif c.owner_parity == 0 and nd > opp_reach:
                p = 10
            elif c.owner_parity == 0:
                p = -5
                ...
                p = 0
            if (c.owner_parity == -pp or c.beacon_parity == -pp) and nd <= 2:
                p -= 20
            if p > best_p:
                best_p = p
                best = d
    ...
    def _safe_fallback_move(self, board, pp, me, opp, painted):
        """Fallback move that ONLY considers collision-safe cells."""
        best = None
        best_p = -999
        for d in Direction.cardinals():
            nxt = me.loc + d
            ...
            if c.is_wall or (nxt == opp.loc and c.owner_parity != pp):
                continue
            if not self._is_collision_safe(board, nxt, pp, opp, painted):
                continue
            # Among safe cells, prefer our own paint
            p = 0
            if c.owner_parity == pp and c.beacon_parity != -pp:
                p = 20
            elif c.owner_parity == 0:
                p = 5
            p += self._md(nxt, opp.loc)
            if p > best_p:
                best_p = p
                best = d
        return Action.Move(best) if best else None
    ...
    def commentate(self, board, player_parity, time_left):
        ...
        mon = self._opp_monitor
        return (
            f"ns3 h={len(p.controlled_hills)}v{len(o.controlled_hills)}"
            ...
            f" oApp={'Y' if mon['approaching'] else 'N'} r={board.current_round}"
        )
