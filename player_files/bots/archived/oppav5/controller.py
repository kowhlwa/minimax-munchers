...
class PlayerController:
    """
    oppav5 — simplified architecture with MST-inspired hill routing.

    Design principles (reverse of prior complexity spiral):
    1. BFS everywhere — no Manhattan distance for movement decisions.
    2. MST-inspired hill visit order — greedy nearest-neighbor pre-plans the
       optimal multi-hill capture route; recomputed only when hill ownership changes.
    3. Anti-oscillation — detect position cycles early (5 turns, not 16) and
       force an escape direction; collision rush only as last resort.
    4. Action validation — filter all Paint actions against current board state
       before returning, preventing invalid-turn losses.
    5. Aggressive stamina use — up to 4 moves when on a hill; regen-aware buffer.
    6. No lookahead complexity — _step_toward (BFS) handles all movement;
       no 2-ply, no sweep order.
    """
    ...
    def __init__(self, player_parity: int, time_left):
        ...
        self._hill_sequence = []       # list of hill IDs in planned visit order
        self._hill_sig = None          # frozenset to detect when to recompute

        # Collision rush (triggered by oscillation only)
        self._rush_mode = False
        self._rush_counter = 0

        # Opponent monitor — updated every turn
        self._opp_monitor = {
            'prev_stamina': None,
            'est_regen': 5,
            'max_moves': 1,
    ...
    def play(
        ...
        # ── 5. COLLISION RUSH (fast oscillation trigger) ────────────────────
        ...
        # Strong commitment: keep targeting a committed hill until it's captured
        ...
        # ── 12. ANTI-OSCILLATION ESCAPE ─────────────────────────────────────
        # If stuck in a cycle and the escape route differs from current target,
        # override to a cell we haven't visited recently.
        ...
        # ── 17. MOVEMENT (up to 4 steps; 4th only on hills) ─────────────────
        ...
        if d1 is not None:
            ...
            if (td > 1 or on_target_hill) and stamina >= GameConstants.EXTRA_MOVE_COST + buf:
                ...
                if d2 is not None:
                    ...
                    if not maze_map and td > 3 and stamina >= GameConstants.EXTRA_MOVE_COST * 2 + buf:
                        ...
                        if d3 is not None:
                            ...
                            # Move 4 (costs 30) — hills only, burn remaining stamina
    ...
    def _oscillation_escape(self, board, pp, pos, opp, my_dist, current_target):
        ...
        Prefers cells that make progress toward current_target over cells that
        don't, but the primary filter is 'not in recent history'. This breaks
        the cycle without discarding the overall strategy.
        """
        ...
        for d in Direction.cardinals():
            ...
            if (nxt.r, nxt.c) in recent:
                continue  # avoid recently visited cells

            # Progress score: BFS distance from nxt to target
            nxt_d = my_dist.get(nxt, 999)
            progress = target_d - nxt_d  # positive = moving toward target
    ...
    def _refresh_hill_sequence(self, board, pp, me, opp, my_dist, opp_dist):
        ...
        MST rationale: rather than picking the best target hill every turn
        independently (which can zigzag), we pre-plan a route over all
        uncaptured hills that minimises total travel. Greedy nearest-neighbor
        (with race-advantage weighting) is a well-known MST approximation for
        the travelling salesman problem and runs in O(hills² * BFS) — fast
        enough to recompute whenever hill ownership changes.

        The sequence is used by _pick_attack_target_sequenced to always target
        the first uncaptured hill in the planned route, giving the bot a
        globally coherent multi-hill strategy.
        """
        ...
        while remaining:
            ...
            for hid, hill in remaining:
                ...
                race_adv = max(0, opp_d - my_d)  # how many steps ahead of opponent we are
                # Lower score = higher priority
                score = float(my_d) - race_adv * 0.5
                # Bonus for stealing opponent's hills
            ...
            # For subsequent hops: approximate BFS from this hill's centroid
            # using manhattan distance (avoids O(hills * board_size) BFS chain).
            # This is the MST approximation trade-off: first hop is exact BFS,
            # subsequent hops use manhattan — good enough for ordering.
            if remaining:
                cells = list(picked.cells)
                cr = sum(l.r for l in cells) // len(cells)
                cc = sum(l.c for l in cells) // len(cells)
                centroid = min(cells, key=lambda l: abs(l.r - cr) + abs(l.c - cc))
                cur_dist = {loc: abs(loc.r - centroid.r) + abs(loc.c - centroid.c)
                            for loc in my_dist}
    ...
    def _pick_attack_target_sequenced(self, board, pp, me, opp, my_dist, opp_dist, dom_needed):
        ...
        Always targets the first uncaptured hill in the pre-planned order.
        Falls back to the score-based _pick_attack_target if the sequence
        has no reachable uncaptured hills.
        """
        for hid in self._hill_sequence:
            ...
            if hill.controller_parity == pp:
                continue  # already ours — advance along sequence
            best_loc = None
            best_d = 999
            for hloc in hill.cells:
                if board.cells[hloc.r][hloc.c].owner_parity != pp:
        ...
        # Fallback: original race-aware score
    ...
    def _validate_paint_actions(self, board, pp, actions):
        ...
        Illegal paints (rejected by the engine):
        - Opponent-owned cells (regardless of layer count)
        - Walls or beacons
        - Our own cells at max paint value
        - Out-of-bounds

        This single final pass prevents the 'invalid turn' loss that occurred
        when the engine rejected a Paint targeting a double-painted opponent cell.
        """
        valid = []
        ...
        for a in actions:
            if isinstance(a, Action.Paint):
                ...
                if c.is_wall or c.beacon_parity != 0:
                    continue
                if c.owner_parity == -pp:
                    continue  # cannot paint opponent cells at all
    ...
    def _pick_hill_cell_to_claim(self, board, pp, hill, my_dist, contested, large_map, opp, stamina):
        ...
        Strongly prefers adjacent cells (d=1) to sweep across the hill
        cell-by-cell, avoiding oscillation by skipping our current standing cell.
        """
        best = None
        best_score = -9999
    ...
    def commentate(self, board, player_parity, time_left):
        ...
        return (
            f"oa5r h={len(p.controlled_hills)}v{len(o.controlled_hills)}"
            ...
            f" r={board.current_round} {rush}{osc} seq={seq}"
        )
