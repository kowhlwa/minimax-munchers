...
class PlayerController:
    """
    nosleep v6.0 — v5 + A* routing + hill reinforcement + anti-oscillation +
    aggressive hill rushing + snipe protection.

    Key improvements over v5:
    1. A* pathfinding (from v3): routes through unpainted hill cells cheaply,
       avoids enemy territory near opponent
    2. Hill reinforcement: paint_hill_cells reinforces friendly hill cells to cap,
       not just neutral cells — critical for defense
    3. Anti-oscillation: _visited_this_turn prevents back-and-forth in multi-move
    4. Aggressive hill rushing: lower buffers, earlier rush activation (td>2),
       more moves per turn toward uncaptured hills
    5. Snipe protection: wider safety margins when opponent has high stamina,
       always end on own paint when opponent can multi-move
    6. Erase on target hill: erase any enemy paint >=1 on hill being captured
    """
    ...
    def play(
        ...
        # === BEACON TRAVEL: teleport to distant hill targets ===
        if (target_cell and target_cell.hill_id != 0
                and td > 6
                and not contested  # opponent not at target = safe to teleport in
                and len(self._our_beacons) >= 2):
            bt = self._try_beacon_travel(board, pp, me, opp, target, my_dist, stamina)
            if bt is not None:
                return bt
        ...
        # === STAMINA BUDGET ===
        _tgt_is_hill = (target_cell and target_cell.hill_id != 0)
        if maze_map:
            ...
            buf = 15
        elif _tgt_is_hill and td > 5:
            buf = 15  # aggressive: save less when far from hill to maximize moves
        else:
            buf = 20
        ...
        # === MOVEMENT MODE ===
        # RUSH: far from target, not on hill, not in danger — maximize moves
        # CAPTURE: near target or on hill — interleave painting
        rushing = td > 2 and not on_target_hill and not danger_zone and not maze_map
        if rushing:
            move_buf = 15
            max_moves = 4
        else:
            move_buf = hill_buf if on_target_hill else buf
            max_moves = 3 if not maze_map else 2

        # === SHOULD WE PAINT TERRITORY EN ROUTE? ===
        paint_territory_en_route = (all_hills_claimed or danger_zone) and not rushing
        trail_layers = 2 if danger_zone else 1

        # === PRE-MOVE PAINTING ===
        # Paint adjacent hill cells at current position (skip when rushing far)
        if not rushing or on_target_hill:
            actions, stamina = self._paint_hill_cells(
                board, pp, pos, actions, stamina, painted, move_buf, reinforce, max_layers)

        # Trail painting before move — skip in rush mode (save stamina for moves)
        if not rushing:
            if paint_territory_en_route or maze_map:
                actions, stamina = self._paint_trail(
                    board, pp, pos, actions, stamina, painted, move_buf, target, trail_layers)
            elif large_map and td > 2:
                actions, stamina = self._paint_trail(
                    board, pp, pos, actions, stamina, painted, move_buf, target)
            elif stamina > 70 and td > 2:
                actions, stamina = self._paint_trail(
                    board, pp, pos, actions, stamina, painted, move_buf, target)

        # === MOVEMENT (loop with paint-ahead for collision safety) ===
        ...
        def _pick_step(from_pos):
            if edge_attack and on_target_hill:
                ...
                return self._astar_step(board, from_pos, target, pp, opp.loc)

        # Track positions visited this turn to prevent oscillation
        _visited_this_turn = {pos}
        cur = pos
        move_count = 0

        for _step_i in range(max_moves):
            # Re-target when sweeping across a hill
            if move_count > 0 and on_target_hill:
                cur_hid = board.cells[cur.r][cur.c].hill_id
                if cur_hid != 0:
                    hill_now = board.hills.get(cur_hid)
                    if hill_now and hill_now.controller_parity != pp:
                        next_cell = self._pick_hill_cell_to_claim(
                            board, pp, hill_now, my_dist, contested, large_map, opp, stamina)
                        if next_cell and next_cell != cur:
                            target = next_cell
            ...
            # Anti-oscillation: don't revisit cells within the same turn
            if nxt in _visited_this_turn:
                for alt_d in Direction.cardinals():
                    if alt_d == d:
                        continue
                    alt_nxt = cur + alt_d
                    if alt_nxt in _visited_this_turn:
                        continue
                    if board.oob(alt_nxt) or board.cells[alt_nxt.r][alt_nxt.c].is_wall:
                        continue
                    d = alt_d
                    nxt = alt_nxt
                    break
                else:
                    break  # all directions revisit — stop

            if board.oob(nxt) or board.cells[nxt.r][nxt.c].is_wall:
                break

            nxt_cell = board.cells[nxt.r][nxt.c]

            # === PAINT-AHEAD: always land on our paint ===
            paint_cost = 0
            if nxt_cell.owner_parity == pp:
                pass  # already ours — safe to step
            elif nxt_cell.owner_parity == 0 and nxt_cell.beacon_parity == 0:
                # Neutral cell — paint it before stepping on
                paint_cost = GameConstants.PAINT_STAMINA_COST
            elif nxt_cell.owner_parity == -pp:
                # Opponent paint — can't pre-paint it
                # Only proceed if opponent is far outside threat range
                if self._md(nxt, opp.loc) > mon['threat_range'] + 2:
                    pass  # opponent far, accept temporary risk
                else:
                    break  # too dangerous, stop here

            # Erase check (thick opponent paint on hill cells)
            erase_cost = 0
            use_erase = False
            if nxt_cell.owner_parity == -pp and nxt_cell.hill_id != 0:
                use_erase = self._should_erase_step(
                    board, pp, nxt, large_map, stamina, move_buf, opp, target_hill_obj)
                if use_erase:
                    erase_cost = GameConstants.ERASE_STEP_EXTRA_COST

            # Total cost for this step
            extra_cost = GameConstants.EXTRA_MOVE_COST * move_count if move_count >= 1 else 0
            total_cost = extra_cost + paint_cost + erase_cost
            if stamina < total_cost + move_buf:
                break
            ...
            # --- Execute move ---
            if use_erase:
                actions.append(Action.Move(d, move_type=MoveType.ERASE))
                stamina -= erase_cost
            else:
            ...
            cur = nxt
            move_count += 1
            _visited_this_turn.add(nxt)

            # Paint hill cells at every stop (critical for capture)
            actions, stamina = self._paint_hill_cells(
                board, pp, cur, actions, stamina, painted,
                move_buf, reinforce, max_layers)

            # Trail painting only in capture mode
            if not rushing:
                if paint_territory_en_route or maze_map or (large_map and td > 3):
                    actions, stamina = self._paint_trail(
                        board, pp, cur, actions, stamina, painted,
                        move_buf, target, trail_layers)

        # === POST-MOVE ===
        final = self._simpos(board, pos, actions)
        actions, stamina = self._paint_hill_cells(
            board, pp, final, actions, stamina, painted,
            max(move_buf - 5, 5), reinforce, max_layers)

        if board.cells[final.r][final.c].hill_id != 0 and stamina > 30:
            actions, stamina = self._paint_trail(
                board, pp, final, actions, stamina, painted, 15, target, trail_layers)
        elif not rushing and (paint_territory_en_route or (large_map and stamina > 25)):
            actions, stamina = self._paint_trail(
                board, pp, final, actions, stamina, painted, 15, target, trail_layers)
        elif rushing and stamina > move_buf + GameConstants.PAINT_STAMINA_COST:
            # Rush mode: only paint with excess stamina at final position
            actions, stamina = self._paint_trail(
                board, pp, final, actions, stamina, painted, move_buf, target)
    ...
    def _update_opp_monitor(self, board, pp, me, opp):
        ...
        # place(0) + travel(10) + walk from beacon (10, 20, 30... after reset)
        if self._opp_beacons:
            ...
            # Cost: 10 for travel, then 10*1 + 10*2 + ... for walks (reset counter)
            beacon_remaining = effective_stamina - GameConstants.EXTRA_MOVE_COST  # minus travel cost
            beacon_walks = 0
            walk_extra = 1
    ...
    def _is_collision_safe(self, board, loc, pp, opp):
        ...
        if mon.get('beacon_threat') and self._opp_beacons:
            for blc in self._opp_beacons:
                ...
                # After teleport, they get beacon_walks steps (10,20,30 cost each)
                # Compute how many walks they can do post-teleport
                eff_stam = min(opp.max_stamina, opp.stamina + mon['est_regen'])
                bcn_rem = eff_stam - GameConstants.EXTRA_MOVE_COST  # travel cost
                bcn_walks = 0
                we = 1
                while bcn_rem > 0:
        ...
        # Snipe protection: high-stamina opponents can afford extra moves
        if mon['stamina'] > 80:
            threat += 1
        ...
        # On neutral cell within threat range = unsafe
        if c.owner_parity == 0 and dist <= threat:
            return False
    ...
    def _should_erase_step(self, board, pp, loc, large_map, stamina, buf, opp, target_hill_obj):
        ...
        Returns True if the cell has opponent paint on a hill and erase is worthwhile."""
        if board.oob(loc):
            return False
        c = board.cells[loc.r][loc.c]
        ...
        # When we're on the hill we're capturing, erase any enemy paint
        # so we can immediately paint behind us
        if target_hill_obj and c.hill_id == target_hill_obj.id:
            if layers >= 1 and stamina >= erase_cost + buf:
                return True
    ...
    def _paint_hill_cells(self, board, pp, pos, actions, stamina, painted, buf, contested=False, max_layers=None):
        """Paint adjacent hill cells — including neutral and friendly cells.

        Paints all reachable hill cells within range that aren't enemy-owned
        or at cap. Prioritises unclaimed cells, then reinforcement."""
        ...
        while stamina - COST >= buf:
            ...
            for d in Direction.cardinals():
                ...
                if is_new:
                    score = 200
                elif c.owner_parity == pp and base + added < cap:
                    # Always reinforce friendly hill cells up to cap
    ...
    def _astar_step(self, board, start, target, pp, opp_loc):
        """A* first-step toward target with hill-aware edge costs.

        Edge costs:
          1.0  — friendly or neutral cell (baseline)
          0.5  — unpainted hill cell we want to capture (incentivise)
          2.0  — enemy-painted non-hill cell near opponent (danger)
          1.5  — enemy-painted cell (mild penalty)
        """
        if start == target:
            return None
        ...
        import heapq
        counter = 0
        open_set = [(0.0, counter, start, None)]
        g_score = {start: 0.0}

        opp_reach = self._opp_monitor['threat_range']

        while open_set:
            f, _, loc, first_dir = heapq.heappop(open_set)

            if loc == target:
                return first_dir

            if f > g_score.get(loc, float('inf')):
                continue
            ...
            for d in Direction.cardinals():
                ...
                cost = 1.0
                on_hill = c.hill_id != 0
                if on_hill and c.owner_parity != pp:
                    cost = 0.5
                elif c.owner_parity == -pp:
                ...
                tentative = g_score[loc] + cost
                if tentative < g_score.get(nxt, float('inf')):
                    g_score[nxt] = tentative
                    fd = first_dir if first_dir is not None else d
                    h = 0.5 * self._md(nxt, target)
                    counter += 1
                    heapq.heappush(open_set, (tentative + h, counter, nxt, fd))
    ...
    def _can_place_beacon_at(self, board, pp, loc):
        """Check if beacon can be placed at loc using 2/3 rule.
        ...
        if c.is_wall:
            return False
        # Tile must be our color or neutral
        if c.owner_parity != pp and c.owner_parity != 0:
            return False
        # Can't place on existing beacon
        if c.beacon_parity != 0:
            return False
        # Don't place on hill cells (beacons don't count for hill control)
        if c.hill_id != 0:
            return False
    ...
    def _try_beacon_travel(self, board, pp, me, opp, target, my_dist, stamina):
        """Teleport via beacon network to reach a distant hill target.

        Flow: walk to nearest own beacon -> BEACON_TRAVEL to beacon nearest
        target -> walk toward target + paint nearby hill cells.
        Only fires if significantly faster than walking and opponent isn't
        ...
        # Don't abandon a hill we're actively contesting
        cur_hid = board.cells[me.loc.r][me.loc.c].hill_id
        if cur_hid != 0:
        ...
        td = my_dist.get(target, 999)
        if td <= 6:
            return None  # close enough to walk

        # Find nearest beacon to us (origin — must stand on it to teleport)
        origin = None
        origin_dist = 999
        for b in self._our_beacons:
            d = my_dist.get(b, 999)
            if d < origin_dist:
                origin_dist = d
                origin = b
        if origin is None or origin_dist > 2:
            return None

        # Find beacon nearest to target (destination — gets consumed on arrival)
        dest = None
        dest_to_target = 999
        for b in self._our_beacons:
            if b == origin:
        ...
        # Don't teleport if opponent is near destination
        if self._md(opp.loc, dest) <= 5:
            return None

        # Must save meaningful turns vs walking
        teleport_steps = origin_dist + 1 + dest_to_target
        if teleport_steps >= td - 2:
            return None

        # Stamina budget: walk-to-beacon + teleport costs
        # Move 0 = free, move 1 = 10, move 2 = 20, ...
        total_moves_min = origin_dist + 1  # walks + teleport
        move_cost = sum(
            GameConstants.EXTRA_MOVE_COST * i
            for i in range(1, total_moves_min)
        )
        if stamina < move_cost + 45:
            return None
        ...
        # Walk to origin beacon
        if cur != origin:
            path = self._find_short_path(board, cur, origin, origin_dist + 1)
            if not path:
                return None
            for d in path:
                actions.append(Action.Move(d))
        ...
        # Beacon travel (destination beacon consumed, leaves 1 paint layer)
        actions.append(Action.Move(
            direction=None,
            move_type=MoveType.BEACON_TRAVEL,
            beacon_target=dest,
        ...
        # Post-teleport: step toward target if budget allows
        moves_used = sum(1 for a in actions if isinstance(a, Action.Move))
        next_cost = GameConstants.EXTRA_MOVE_COST * moves_used
        remaining = stamina - move_cost
        if remaining > next_cost + 35:
            step = self._step_toward(board, cur, target, pp, opp.loc,
                                     hill_target=True)
            if step:
                nxt = cur + step
                if not board.oob(nxt) and not board.cells[nxt.r][nxt.c].is_wall:
        ...
        # Paint neutral hill cells adjacent to final position
        paint_budget = remaining - 20
        for d in Direction.cardinals():
            adj = cur + d
            if board.oob(adj):
                continue
            c = board.cells[adj.r][adj.c]
            if (c.hill_id != 0 and c.owner_parity == 0
                    and not c.is_wall and c.beacon_parity == 0
                    and paint_budget >= GameConstants.PAINT_STAMINA_COST):
                actions.append(Action.Paint(adj))
                paint_budget -= GameConstants.PAINT_STAMINA_COST

        # Safety: consumed beacon leaves our paint so landing cell is safe,
        # but if we walked further verify final position
        if not self._is_collision_safe(board, cur, pp, opp):
            return None
    ...
    def _try_place_beacon(self, board, pp, actions, start_pos, me, opp, our_hills, total_hills):
        ...
        Strategic: place near captured hills as teleport anchors for rotation.
        ...
        if our_hills == 0:
            return actions
        # Cap at 2 active beacons (more = wasted paint from 3x3 consumption)
        if len(self._our_beacons) >= 2:
            return actions
        ...
        # Anti-clustering: don't place if one of ours is already nearby
        for b in self._our_beacons:
            if self._md(final, b) <= 4:
                return actions

        # Place near any hill we control (within 3 tiles)
        near_our_hill = False
        for hid in me.controlled_hills:
            hill = board.hills[hid]
            for hloc in hill.cells:
                if self._md(final, hloc) <= 3:
                    near_our_hill = True
                    break
            if near_our_hill:
                break
    ...
    def commentate(self, board, player_parity, time_left):
        ...
        return (
            f"ns6.0 h={len(p.controlled_hills)}v{len(o.controlled_hills)}"
