...
class PlayerController:
    ...
    def __init__(self, player_parity: int, time_left):
        ...
        self._last_territory = 0  # track territory growth
        self._territory_stale_turns = 0  # how many turns territory hasn't grown
    ...
    def play(
        ...
        # === TERRITORY STAGNATION: if territory hasn't grown, expand instead of camping ===
        cur_territory = board.get_territory_count(pp)
        if cur_territory <= self._last_territory:
            self._territory_stale_turns += 1
        else:
            self._territory_stale_turns = 0
        self._last_territory = cur_territory

        # After 15 turns of no territory growth, force expansion or unclaimed hill capture.
        # This prevents the 1700-turn camping pattern seen in long games.
        if self._territory_stale_turns >= 15:
            unclaimed = self._find_unclaimed_far_hill(board, pp, my_dist, opp_dist, opp.loc)
            if unclaimed:
                self._territory_stale_turns = 0
                return self._rush_to(board, pp, me, opp, my_dist, stamina, unclaimed)
            # No unclaimed hills — expand territory for tiebreak
            if self._territory_stale_turns >= 30:
                self._territory_stale_turns = 0
                return self._play_territory(board, pp, me, opp, my_dist, stamina)
